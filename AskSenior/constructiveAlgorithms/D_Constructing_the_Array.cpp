...
struct comp {
    bool operator()(const pair<int, pair<int, int>>& a, const pair<int, pair<int, int>>& b) {
        if (a.first == b.first) {
            return a.second.first > b.second.first; // Smaller value for second->first
        }
        return a.first < b.first; // Larger value for first
    }
};
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        while(i<=n){
            ...
            int mid = it.first;
            if(mid&1){
                int index= (it.second.first + it.second.second) /2;
                v[index]=i;
                if(index-1>=it.second.first){
                    pq.push({index-it.second.first, {it.second.first,index-1}});
                }
                if(index+1<=it.second.second){
                    pq.push({it.second.second-index, {index+1,it.second.second}});
                }
            }
            else{
                int index= (it.second.first + it.second.second-1) /2;
                v[index]=i;
                if(index-1>=it.second.first){
                    pq.push({index-it.second.first, {it.second.first,index-1}});
                }
