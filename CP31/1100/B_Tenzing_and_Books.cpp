...
bool check(string bitmask, vi&a,vi&b,vi&c,int x){
    // int i=0;
    int n= a.size();
    int t=0;
    for(int i=0;i<n;i++){
        if(bitmask[0]=='1')t|=a[i];
        if(t==x) return true;
        if(bitmask[1]=='1')t|=b[i];
        if(t==x) return true;
        if(bitmask[2]=='1')t|=c[i];
        if(t==x) return true;
        if(t>x) return false;
...
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        for (int mask = 0; mask < (8); mask++) {
            string bitmask = "";
            for (int i = n - 1; i >= 0; i--) {
                bitmask += ((mask >> i) & 1) ? '1' : '0';
            }
            // bitmasks.push_back(bitmask);
            if(check(bitmask,a,b,c,x)){
                py;
                return;
            }
        }
