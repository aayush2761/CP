...
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        if(a <= k){
            ...
            ll cnt = 1e18;
            for(int i=0; i<k; i++){
                cnt = min(cnt,(ll) abs(arr[b-1].first - arr[i].first) 
                             + abs(arr[b-1].second - arr[i].second));
            }
            cnt = min(cnt, abs(arr[a-1].first - arr[b-1].first) + 
            ...
            ll cnt = 1e18;
            for(int i=0; i<k; i++){
                cnt = min(cnt,(ll) abs(arr[a-1].first - arr[i].first) 
            ...
            ll cnt1 = 1e18;
            for(int i=0; i<k; i++){
                cnt1 = min(cnt1, abs(arr[b-1].first - arr[i].first) 
                             + abs(arr[b-1].second - arr[i].second));
            }
    
            ll cnt2 = 1e18;
            for(int i=0; i<k; i++){
                cnt2 = min(cnt2, abs(arr[a-1].first - arr[i].first) 
                             + abs(arr[a-1].second - arr[i].second));
            }
            // cout<<cnt1<<' '<<cnt2<<'\n';
            ll cnt = min(cnt1 + cnt2, abs(arr[a-1].first - arr[b-1].first) + 
                           abs(arr[a-1].second - arr[b-1].second));
            cout<<cnt<<'\n';
        }
