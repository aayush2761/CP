...
void check(int mid, ll total, vi &v, bool &flag1, bool &resFlag) {
    ll count = 0;
    foreach(i, 0, v.size(), 1) {
        ll val = (v[i] + mid);  // Ensure calculations stay in long long
        count += val * val;
        if (count > total) {
            flag1 = false;
            return;
...
int32_t main() {
    ...
    auto solve = [&]() {
        ...
        vi v(n);
        cin >> v;
        int low = 1, high = sqrt(total);  // Optimized upper bound
        bool flag1 = false, resFlag = false;

        while (low <= high) {
            int mid = low + (high - low) / 2;
            check(mid, total, v, flag1, resFlag);
            if (flag1) {
                if (resFlag) {
                    out(mid/2);
                    return;
